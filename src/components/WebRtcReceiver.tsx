  ...
  return (
    <section className="workspace">
      ...
      <div className="transfer-card receiver-layout">
        <div className="panel">
          {phase === "pairing" ? <div className="pairing-entry"><span className="pairing-icon"><Link2 size={28} /></span><h2>Enter pairing code</h2><p>Use the six digits shown on the sending device.</p><input className="pairing-input" value={pairingCode} onChange={(event) => setPairingCode(normalizePairingCode(event.target.value))} inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} placeholder="000000" aria-label="Six-digit pairing code" /><button className="button button-primary button-wide" onClick={() => void connect()} disabled={!isValidPairingCode(pairingCode)}><Link2 size={16} />Connect to sender</button></div> : phase === "preparing" ? <div className="qr-placeholder"><div className="qr-placeholder-inner"><span className="spinner dark-spinner" /><strong>Finding sender and creating connection</strong><span>The local runtimes are exchanging WebRTC connection details over this LAN.</span></div></div> : <div className="qr-placeholder"><div className="qr-placeholder-inner">{phase === "saved" ? <FolderCheck size={48} /> : phase === "failed" ? <AlertTriangle size={48} /> : <Database size={48} />}<strong>{phase === "saved" ? "Files reconstructed" : phase === "failed" ? "Pairing failed" : connected ? "Devices connected" : "Pairing complete"}</strong><span>{phase === "failed" ? "Reconnect and enter the sender code again." : "File bytes are written to private browser storage before acknowledgement."}</span></div></div>}
        </div>
